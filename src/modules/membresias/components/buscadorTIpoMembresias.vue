...
const buscarTexto = (texto) => {
    if(texto.length <= 0){
        ...
        store.buscarTipoMembresiasPorNombre(texto).then((res) => {
            if(res){
...
<template>
    <div class="buscador">
        <input type="text" placeholder="Buscar tipo membresía por nombre" v-model="txtBusqueda" @keyup.enter="buscarTexto(txtBusqueda)">
