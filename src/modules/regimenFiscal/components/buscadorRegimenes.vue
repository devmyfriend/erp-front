<template>
    <div class="contenedor">
        <input type="text" placeholder="Regimen Simplificado" v-model="txtBusqueda" @keyup.enter="buscar(txtBusqueda)">
...
const { useRegimenFiscal } = require('../store/regimenesFiscales.js')
...
function buscar(txt) {
    ...
    if (txtBusqueda.value === '' || txtLargo.value == 0) {
        ...
        store.buscarRegimenFiscal(txtBusqueda.value).then(() => {
            emit('eBusqueda', txt);
        });
    }
    txtBusqueda.value = '';
